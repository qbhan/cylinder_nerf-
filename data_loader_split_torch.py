...
from nerf_sample_ray_split_torch import RaySamplerSingleImage
import glob
import torch
...
def load_single_data(basedir, scene, split, skip=1, try_load_min_depth=True, only_img_files=False):
...
def load_single(img_files, intrinsics_files, pose_param_net, i):
    def parse_txt(filename):
        assert os.path.isfile(filename)
        nums = open(filename).read().split()
        return np.array([float(x) for x in nums]).reshape([4, 4]).astype(np.float32)
    ...
    intrinsics = parse_txt(intrinsics_files[i])
    pose = pose_param_net(i)


    max_depth = None

    return RaySamplerSingleImage(H=H, W=W, intrinsics=intrinsics, c2w=pose,
                                                img_path=img_files[i],
                                                max_depth=max_depth)
