...
from ddp_model import NerfNetWithAutoExpo, LearnPose
import time
...
def create_nerf(rank, args):
    ...
    for m in range(models['cascade_level']):
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optim, milestones=list(range(0, 10000, 10)), gamma=0.9954)
        models['net_{}'.format(m)] = net
        models['optim_{}'.format(m)] = optim
        models['sched_{}'.format(m)] = scheduler
...
def ddp_train_nerf(rank, args):
    ...
    pose_param_net, optimizer_pose, scheduler_pose = create_posenet(rank, args)
    ...
    for global_step in range(start+1, start+1+args.N_iters):
        ...
        epoch = global_step//len(img_files)
        # stable training for some epochs
        if epoch < start_refine_epoch:
            pose_param_net.eval()
            with torch.no_grad():
        ...
        for m in range(models['cascade_level']):
            ...
            optim.zero_grad()
            if m==1: optimizer_pose.zero_grad()
            # print(bg_z)
